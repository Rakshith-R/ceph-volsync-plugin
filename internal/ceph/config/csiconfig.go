...
// GetRBDMirrorDaemonCount returns the number of mirror daemon count
// for the given clusterID.
func GetRBDMirrorDaemonCount(
	pathToConfig, clusterID string,
) (int, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
...
// GetRBDNetNamespaceFilePath returns the netNamespaceFilePath
// for RBD volumes.
func GetRBDNetNamespaceFilePath(
	pathToConfig, clusterID string,
) (string, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
...
// GetCephFSNetNamespaceFilePath returns the netNamespaceFilePath
// for CephFS volumes.
func GetCephFSNetNamespaceFilePath(
	pathToConfig, clusterID string,
) (string, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
...
// GetNFSNetNamespaceFilePath returns the netNamespaceFilePath
// for NFS volumes.
func GetNFSNetNamespaceFilePath(
	pathToConfig, clusterID string,
) (string, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
...
// GetCrushLocationLabels returns the `readAffinity.enabled` and
// `readAffinity.crushLocationLabels` values from the CSI config for
// the given `clusterID`. If `readAffinity.enabled` is set to true it
// returns `true` and `crushLocationLabels`, else returns `false` and
// an empty string.
func GetCrushLocationLabels(
	pathToConfig, clusterID string,
) (bool, string, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
	...
	crushLocationLabels := strings.Join(
...
// GetCephFSMountOptions returns the `kernelMountOptions` and
// `fuseMountOptions` for CephFS volumes.
func GetCephFSMountOptions(
	pathToConfig, clusterID string,
) (string, string, error) {
	cluster, err := readClusterInfo(pathToConfig, clusterID)
	if err != nil {
	...
	return cluster.CephFS.KernelMountOptions,
		cluster.CephFS.FuseMountOptions, nil
}
