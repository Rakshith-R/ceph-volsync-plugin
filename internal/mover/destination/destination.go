...
func (s *DataServer) Sync(stream grpc.ClientStreamingServer[apiv1.SyncRequest, apiv1.SyncResponse]) error {
	var file *os.File
	var fullPath string
...
func (s *DataServer) writeBlocks(file *os.File, fullPath string, req *apiv1.WriteRequest) error {
	...
	if maxSize > fileInfo.Size() {
		if err := file.Truncate(maxSize); err != nil {
			s.logger.Error(err, "Failed to truncate file", "path", fullPath, "size", maxSize)
			return fmt.Errorf("failed to resize file %s to %d bytes: %w", fullPath, maxSize, err)
		}
		s.logger.Info("Resized file", "path", fullPath, "old_size", fileInfo.Size(), "new_size", maxSize)
	}

	// Process each block
	for i, block := range req.Blocks {
		if _, err := file.Seek(int64(block.Offset), 0); err != nil {
			s.logger.Error(err, "Failed to seek", "path", fullPath, "offset", block.Offset, "block_index", i)
			return fmt.Errorf("failed to seek to offset %d in %s: %w", block.Offset, fullPath, err)
		}

		if block.IsZero {
			zeros := make([]byte, block.Length)
			if _, err := file.Write(zeros); err != nil {
				s.logger.Error(err, "Failed to write zeros", "path", fullPath, "offset", block.Offset, "length", block.Length, "block_index", i)
				return fmt.Errorf("failed to write zeros at offset %d in %s: %w", block.Offset, fullPath, err)
			}
			s.logger.V(1).Info("Wrote zero block", "path", fullPath, "offset", block.Offset, "length", block.Length)
		} else {
			if _, err := file.Write(block.Data); err != nil {
				s.logger.Error(err, "Failed to write data", "path", fullPath, "offset", block.Offset, "length", len(block.Data), "block_index", i)
				return fmt.Errorf("failed to write data at offset %d in %s: %w", block.Offset, fullPath, err)
			}
			s.logger.V(1).Info("Wrote data block", "path", fullPath, "offset", block.Offset, "length", len(block.Data))
		}
	}
...
func (s *DataServer) Delete(ctx context.Context, req *apiv1.DeleteRequest) (*apiv1.DeleteResponse, error) {
