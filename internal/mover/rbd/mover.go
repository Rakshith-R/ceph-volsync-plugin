...
const (
	...
	volSyncRBDPrefix = mover.VolSyncPrefix + "rbd-"
...
// Mover is the reconciliation logic for the RBD-based data mover.
type Mover struct {
...
func (m *Mover) Name() string { return rbdMoverName }
...
func (m *Mover) ensureServiceAndPublishAddress(ctx context.Context) (bool, error) {
	...
	service := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      volSyncRBDPrefix + m.direction() + "-" + m.owner.GetName(),
...
func (m *Mover) ensureSecrets(ctx context.Context) (*string, error) {
	...
	keySecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      volSyncRBDPrefix + m.owner.GetName(),
			Namespace: m.owner.GetNamespace(),
		},
	}
...
func (m *Mover) ensureCephCSISecret(
	...
	// Look up the secret ref from csi config (RBD variant)
	secretName, secretNS, err :=
		config.GetRBDControllerPublishSecretRef(
...
func (m *Mover) serviceSelector() map[string]string {
	return map[string]string{
		...
		"app.kubernetes.io/component": "rbd-mover",
...
func (m *Mover) ensureJob(
	...
	op, err := utils.CreateOrUpdateDeleteOnImmutableErr(ctx, m.client, job, logger, func() error {
		...
		// Add MOVER_TYPE environment variable
		...
		// Disable rsync tunnel for RBD mover
		containerEnv = append(containerEnv, corev1.EnvVar{
		...
		if m.vh.IsCopyMethodDirect() {
			...
			if err != nil {
				logger.Error(err, "unable to determine proper affinity",
		...
		// Update the job securityContext, podLabels and resourceRequirements from moverConfig
		utils.UpdatePodTemplateSpecFromMoverConfig(
			&job.Spec.Template, m.moverConfig, corev1.ResourceRequirements{},
		)
	...
	if job.Status.Failed >= *job.Spec.BackoffLimit {
		...
		utils.UpdateMoverStatusForFailedJob(ctx, m.logger, m.latestMoverStatus,
			job.GetName(), job.GetNamespace(), rsynctls.LogLineFilterFailure)

		logger.Info("deleting job -- backoff limit reached")
		m.eventRecorder.Eventf(m.owner, job, corev1.EventTypeWarning,
			volsyncv1alpha1.EvRTransferFailed, volsyncv1alpha1.EvADeleteMover,
			"mover Job backoff limit reached")
	...
	if op == ctrlutil.OperationResultCreated {
		...
		m.eventRecorder.Eventf(m.owner, job, corev1.EventTypeNormal,
			volsyncv1alpha1.EvRTransferStarted, volsyncv1alpha1.EvACreateMover,
			"starting %s to %s data", utils.KindAndName(m.client.Scheme(), job), dir)
	}
	...
	utils.UpdateMoverStatusForSuccessfulJob(ctx, m.logger, m.latestMoverStatus,
		job.GetName(), job.GetNamespace(), rsynctls.LogLineFilterSuccess)
