...
2. **Read**: Parallel workers (`ReadWorkers`) read raw data from disk/device via `DataReader.ReadAt()`
   - Detects all-zero blocks early and routes them separately
...
- **`ReadChunk`**: Adds raw `Data []byte` and resource tracking
- **`ZeroChunk`**: Detected all-zero block (no data payload)
- **`HashedChunk`**: Adds `Hash [32]byte` (SHA-256)
- **`CompressedChunk`**: LZ4-compressed data with `UncompressedLength` and `IsRaw` flag
